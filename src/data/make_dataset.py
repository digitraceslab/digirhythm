...
def main():
    ...
    features = pd.read_csv(
        processed_path + f"{study}_all_features_{frequency}.csv",
        dtype={"subject_id": str},
    )

    res = survey.merge(features, on=["subject_id", "date"], how="left").dropna(
        subset=features_set
    )
    ...
    if frequency == "7ds":
        # Sort by date first
        similarity_baseline_df = similarity_baseline_df.sort_values(
            ["subject_id", "date"]
        )
        similarity_baseline_df.set_index("date", inplace=True)
        similarity_baseline_df = (
            similarity_baseline_df.groupby("subject_id")
            .rolling(7)
            .agg("median")
            .reset_index()
            .dropna()
    ...
    # NOrmalize age
    res["age:norm"] = (res["age"] - res["age"].min()) / (
        res["age"].max() - res["age"].min()
    )
